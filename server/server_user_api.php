...
//if there is a validated session (user or admin)
else if (Session::getInstance()->isAtLeastUser() === true) {

    //if the crfs token is valid
    if (isTokenValid() === true) {

        //performs actions that need at least a validated user session
        $response = actionValidatedUser($request, $response);
...
/* -------------------------------------------------------------------------- */
/*            api calls allowed for a validated user                          */
/* -------------------------------------------------------------------------- */

function actionValidatedUser($request, $response) {
        ...
        case 'POST':
            if (isset($_POST['action'])) {
                switch ($_POST['action']) {
                    ...
                    case 'processInternalMail':
                        if (isset($_POST['senderName'], $_POST['senderEmail'], $_POST['content'])) {
                            $response = $request->processInternalMail($_POST['senderName'], $_POST['senderEmail'], $_POST['content']);
                        }
                        break;                                                
        ...
        case 'PUT':
            ...
            if (isset($_PUT['action'])) {
                switch ($_PUT['action']) {            
                    ...
                    case 'changePassword':
                        if (isset($_PUT['currentPwd'], $_PUT['newPwd'])) {
                            $response = $request->changePassword($_PUT['currentPwd'], $_PUT['newPwd']);
                        }
                        break;                     
